...
class Prestamista
{
  ...
  public function registrarCuentaBancaria($tipo_cuenta, $saldo, $cod_cliente)
  {
    ...
    $sql = "INSERT INTO cuentabancaria VALUES (:n1, NOW(), :n2, :n3, 'En linea', :n4);";
    try {
      $stmt = $con->conectar()->prepare($sql);
  ...
  public function obtenerTransferenciasPorCliente($cod_cliente)
  {
    global $con;
    $movimientos = [];
    $sql = "SELECT t.numTransferencia, t.fechaTransferencia, t.montoTransferencia, t.cuentaDestino, t.conceptoTransferencia, t.numCuenta
    FROM transferencias AS t
    JOIN cuentabancaria as c
    ON c.numCuenta = t.numCuenta
    JOIN cliente as cl 
    ON cl.codigo_cliente = c.codigo_cliente
    WHERE cl.codigo_cliente = :n1
